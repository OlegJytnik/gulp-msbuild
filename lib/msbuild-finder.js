...
var detectMsBuild16Dir = function(pathRoot) {
  var vs2019Path = path.join(pathRoot, 'Microsoft Visual Studio', '2019');
  var possibleFolders = ['BuildTools', 'Enterprise', 'Professional', 'Community'];

  for(var index = 0; index < possibleFolders.length; index++) {
    try {
      var folderPath = path.join(vs2019Path, possibleFolders[index]);
      fs.statSync(folderPath);
      return folderPath;
    } catch (e) { }
  }
...
module.exports.find = function (options) {
  ...
  if (options.toolsVersion === 'auto') {
  ...
  } else if (options.toolsVersion === 15.0) {
    ...
    } else if (options.toolsVersion === "Current") {
      var msbuildDir = detectMsBuild16Dir(pathRoot);
      if (msbuildDir) {
        msbuildRoot = msbuildDir;
      } else {
        msbuildRoot = pathRoot;
  ...
  if (version === '12.0' || version === '14.0' || version === '15.0') {
    ...
    return path.join(msbuildRoot, 'MSBuild', version, 'Bin', x64_dir, 'MSBuild.exe');
	  ...
	  var x64_dir = is64Bit ? 'amd64' : '';
    return path.join(msbuildRoot, 'MSBuild', 'Current', 'Bin', x64_dir, 'MSBuild.exe');
  } else {
